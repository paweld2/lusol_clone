            ...
            int *NSING, int IPVT[], int IX[])
{
  ...
  int  I, J, K, KP1, L, LAST, LENCOL, IMAX, JMAX, JLAST, JNEW, IDA1, IDA2;
  REAL T, AIJMAX, AJMAX;
...
void LU1DPP(LUSOLrec *LUSOL, REAL DA[], int LDA, int M, int N, REAL SMALL,
            int *NSING, int IPVT[], int IX[])
{
  int  I, J, K, KP1, L, LAST, LENCOL, IDA1, IDA2;
  REAL T;
...
void LU1MXC(LUSOLrec *LUSOL, int K1, int K2, int IX[])
{
  ...
  for(K = K1; K <= K2; K++) {
    ...
    if(LENJ==0)
#ifdef YZHANG
      /* XXX: avoiding overwriting the next column when current
         column is empty (i.e. LENJ==0) */
      ;
#else
      LUSOL->a[LC] = ZERO;
#endif      
