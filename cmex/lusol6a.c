...
/* ==================================================================
   ...
   ================================================================== */
#define UseVersion20040510
#ifdef UseVersion20040510
...
void LU6CHK(LUSOLrec *LUSOL, int MODE, int LENA2, int *INFORM)
{
  ...
  LPRINT = LUSOL->luparm[LUSOL_IP_PRINTLEVEL];
  KEEPLU = (MYBOOL) (LUSOL->luparm[LUSOL_IP_KEEPLU] != 0);
  TRP    = (MYBOOL) (LUSOL->luparm[LUSOL_IP_PIVOTTYPE] == LUSOL_PIVOT_TRP);
  NRANK  = LUSOL->luparm[LUSOL_IP_RANK_U];
  LENL   = LUSOL->luparm[LUSOL_IP_NONZEROS_L];
  UTOL1  = LUSOL->parmlu[LUSOL_RP_SMALLDIAG_U];
  UTOL2  = LUSOL->parmlu[LUSOL_RP_EPSDIAG_U];
...
void LU1L0T(LUSOLrec *LUSOL, LUSOLmat **mat)
{
...
#ifdef MATLAB  
  lsumr = (int *) mxMalloc((LUSOL->m+1)*sizeof(*lsumr));
#else
  lsumr = (int *) malloc((LUSOL->m+1)*sizeof(*lsumr));
#endif
  /* Compute row non-zero counts */
#ifdef YZHANG
  /* XXX: otherwise we get seg fault when (*mat)->vlen is empty */
  MEMCLEAR(((*mat)->vlen+1), LUSOL->m);
#else
  ...
  for(K = 1; K <= NUML0; K++) {
    ...
    L = L1;
    L1 -= LEN;
    I = LUSOL->indr[L1];    /* Obtain the (permuted) column index */
    for(; LEN > 0; LEN--) {
      L--;
      J = LUSOL->indc[L];
      LL = lsumr[J]++;
...
#ifdef UseRowBasedL0
  LU6L0T_v(LUSOL, LUSOL->L0, V, NZidx);
