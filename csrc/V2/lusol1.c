...
/*     ========================================================
       ...
       ======================================================== */
        ...
        for(I = 1; I <= K-1; I++) {
          T = DA[DAPOS(I,LAST)];
          DA[DAPOS(I,LAST)] = DA[DAPOS(I,J)];
          DA[DAPOS(I,J)] = T;
        }
        for(I = K; I <= M; I++) {
          T = DA[DAPOS(I,LAST)];
          DA[DAPOS(I,LAST)] = ZERO;
          DA[DAPOS(I,J)] = T;
        }
        LAST--;
        if(J<=LAST)
...
/*      ------------------------------------------------------------------
        ...
        ------------------------------------------------------------------ */
#ifdef LUSOLFastCopy
  MEMCPY(LUSOL->a+1,D+1,LEND);
//  MEMMOVE(LUSOL->a+1,D+1,LEND);
#else
...
/*      Threshold Partial   Pivoting (normal). */
  TPP = (MYBOOL) (LPIV==LUSOL_PIVOT_TPP);
/*      Threshold Rook      Pivoting */
  TRP = (MYBOOL) (LPIV==LUSOL_PIVOT_TRP);
/*      Threshold Complete  Pivoting. */
  TCP = (MYBOOL) (LPIV==LUSOL_PIVOT_TCP);
/*      Threshold Symmetric Pivoting. */
  TSP = (MYBOOL) (LPIV==LUSOL_PIVOT_TSP);
  DENSLU = FALSE;
  MAXROW = MAXCOL-1;
