...
/* Overall dimensions allocated */
  int    maxm = MAXROWS, maxn = MAXCOLS, maxnz = MAXNZ, maxlu = MAXLU,
         replace = 0, randcol = 0;
  MYBOOL ftran = TRUE;
...
/* Define local storage variables */
  ...
  REAL Amax  , test  ,
       bnorm , rnorm , xnorm, 
       *rhs  , *r    , *x;
  char fileext[50], filename[255];
  MYBOOL printsolution = FALSE, success = FALSE;

/* Check if we have input parameters */
  useropt = argc;
  if(useropt < 2) {
    printf("LUSOL v2.0 - Linear equation solver - Development edition.\n");
...
/* -----------------------------------------------------------------
   ...
   ----------------------------------------------------------------- */
  fprintf(stdout, "\n===================================\n");
  fprintf(stdout,   "LUSOL v2.0 - Linear equation solver");
  fprintf(stdout, "\n===================================\n");
...
Resolve:
#if 1
  MEMCPY(x, b, lenb+BASE);
  if(ftran)
    inform = LUSOL_ftran(LUSOL, x);
  else
    inform = LUSOL_btran(LUSOL, x);
#else
  MEMCPY(rhs, b, lenb+BASE);
  if(ftran)
    LU6SOL( LUSOL, LUSOL_SOLVE_Aw_v, rhs, x, &inform );
  else
    LU6SOL( LUSOL, LUSOL_SOLVE_Atv_w, x, rhs, &inform );
#endif
  if (inform > LUSOL_INFORM_SERIOUS) {
    fprintf(outunit, "Error:\n%s\n", LUSOL_informstr(LUSOL, inform));
    goto x900;
 ...
 /* Check if we should replace a column and resolve */ 
  if(replace > 0) {
    ...
    if(mod(replace, 2) == 1) {
      ...
      rnorm   = rand();
      randcol = (int) (n * rnorm / RAND_MAX) + 1;
#if 1
      MEMCLEAR(x, m+1);
      for(i = 1; i < m; i++)
