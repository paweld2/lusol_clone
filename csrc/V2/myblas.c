...
x20:
  if(DOFASTMATH) {
    memcpy(&dy[1], &dx[1], n*sizeof(REAL));
    return;
  }
...
/* copies a scalar, a, to a vector, x.
   ...
   comments.  To make a REAL precision version interchange
...
/* ------------------------------------------------------------------
  ...
  for (ix = 1; ix<=1+(n-1)*incx; ix=ix+incx) {
     seeds[1]     = 171*local_mod(seeds[1], 177) -  2*(seeds[1]/177);
     seeds[2]     = 172*local_mod(seeds[2], 176) - 35*(seeds[2]/176);
     seeds[3]     = 170*local_mod(seeds[3], 178) - 63*(seeds[3]/178);
