...
#elif 0
  LUSOL->luparm[LUSOL_IP_USEROWL0]   = LUSOL_AUTOORDER;
#endif
...
/* Create the arrays */
  ...
  Aij = (REAL *) calloc(maxnz + BLAS_BASE, sizeof(REAL));
  iA = (int *)   calloc(maxnz + BLAS_BASE, sizeof(int));
  jA = (int *)   calloc(maxnz + BLAS_BASE, sizeof(int));
  if(ftran)
    lenb = maxm;
  else
    lenb = maxn;
  b   = (REAL *) calloc(lenb+BLAS_BASE, sizeof(REAL));
  ...
  r = (REAL *) calloc(lenx+BLAS_BASE, sizeof(REAL));
  x = (REAL *) calloc(lenx+BLAS_BASE, sizeof(REAL));
